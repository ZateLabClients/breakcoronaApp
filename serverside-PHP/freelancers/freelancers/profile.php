...
<?php
  ...
  require '../connect.php';
  $id=$_SESSION["id"];
  $sql="Select id,name,job,place,phone,status,wages from freelancers where id=$id";
  $result=$conn->query($sql);
  $row=$result->fetch_assoc();
  $loc=urlencode($row["place"]);
  $conn->close();
...
<h4>About</h4>
  ...
  echo "Job:".$row["job"]."<br>";
  echo "Address:".$row["place"]."<br>";
  echo "Minimum wages:&#x20b9;".$row["wages"]."<br>";
